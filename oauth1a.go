...
// ClientConfig is a container for client-specific configuration related to the OAuth process.
...
// GetOAuthParams returns a map of all of the oauth_* (including signature) parameters for the
...
func (s *HmacSha1Signer) GetOAuthParams(request *http.Request, clientConfig *ClientConfig, userConfig *UserConfig, nonce string, timestamp string) (map[string]string, string, error) {
	...
	return oauthParams, signatureBase, nil
}

// GetSignature calculates the HMAC-SHA1 signature of a base string, given a
// consumer and token secret.
...
// Sign adds the appropriate OAuth Authorization header to
// given an unsigned request using the HMAC-SHA1 algorithm.
func (s *HmacSha1Signer) Sign(request *http.Request, clientConfig *ClientConfig, userConfig *UserConfig) error {
	...
	oauthParams, _, err := s.GetOAuthParams(request, clientConfig, userConfig, nonce, timestamp)
	if err != nil {
		return err
	}
...
// Rfc3986Escape escapes a string more in line with Rfc3986 than http.URLEscape.
